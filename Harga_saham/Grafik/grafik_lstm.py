...
ax1.plot(pred_real, label='Prediksi LSTM', color='blue', linestyle='--')
ax1.set_title('Prediksi Harga Saham BBCA - LSTM')
...
ax2.fill_between(range(len(error)), error, color='blue', alpha=0.3)
ax2.axhline(y=0, color='black')
ax2.set_title('Residual Error LSTM')
ax2.set_xlabel('Hari')
...
plt.savefig('grafik_lstm.png', dpi=300, bbox_inches='tight')

plt.show()

print(f"Harga Asli Terakhir : Rp {y_test_real[-1]:.2f}")
print(f"Prediksi LSTM      : Rp {pred_real[-1]:.2f}")